...
export default function LeaveBannersPage() {
  ...
  const { data: banners, isLoading } = useQuery({
    queryKey: ['leave-banners', guildId],
    queryFn: async () => {
      const { data } = await apiClient.leaveBanners.list(guildId)
      return data as any[]
    }
  })
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      ...
      formDataObj.append('name', 'Leave Banner')
      ...
      if (banner) {
        return await apiClient.leaveBanners.update(guildId, banner.id, formData)
      } else {
        return await apiClient.leaveBanners.create(guildId, formDataObj)
      }
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['leave-banners', guildId] })
      setMediaFile(null)
  ...
  const previewMutation = useMutation({
    mutationFn: async () => {
      const { data } = await client.post(`/guilds/${guildId}/leave/preview`, { text: formData.text })
      return data
    },
    onSuccess: (data) => {
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-bold">Leave Banner</h1>
      ...
      <form onSubmit={(e) => { e.preventDefault(); saveMutation.mutate() }} className="bg-background-primary border border-background-tertiary rounded-lg p-6 space-y-4">
        ...
        <div>
          ...
          <textarea
            ...
            placeholder="Goodbye {user_mention}!"
            required
            rows={6}
            className="w-full px-3 py-2 bg-background-secondary border border-background-tertiary rounded-lg focus:outline-none focus:ring-2 focus:ring-brand-primary"
          />
